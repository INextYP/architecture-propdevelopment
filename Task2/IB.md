В чек-лист включены следующие раделы:

- управление доступом, так как есть явные проблемы с контролем доступа

- безопасность данных из-за большого количества ПД

- защита инфраструктура, так как есть шансы в потенциальных угрозах из-за недостаточной сегментации

- управление инфидентами - для исключения факторов, которые могут повлиять на финансовые убытки или на репутационные потери

- специфические аспекты - так как есть необходимость к управлению доступом к физ. ресурсам и их подсчет
...
| 2   | Настроены ли требования к сложности паролей и их регулярное обновление?                                                                  |  нет     |   отсутствует политика           |
...
| 6   | Ведётся ли журналирование всех операций доступа к системам и данным? Кто имеет доступ к журналам?                                       |  нет     |     журналирование частисное         |
| 7   | Используются ли механизмы контроля доступа на основе атрибутов для повышения гибкости управления доступом?                              |   нет    |              |
| 8   | Используются ли системы аутентификации пользователей для доступа к системам?                                                             |   нет    |     частично, нет единой системы         |
| 9   | Созданы ли замкнутые контуры для подсистем, работающих с чувствительными данными?                                                        |   нет    |              |
| 10  | Ограничен ли доступ к системам из внешних источников?                                                                                    |   нет    |    частично. Межсетевой экран есть, но явно нужна его проверка          |
...
| 2   | Используется ли шифрование на уровне базы данных, файловой системы и при передаче данных по сети?                                       |   нет    |   частично. используется выборочно           |
...
| 6   | Ограничен ли доступ к резервным копиям данных, и зашифрованы ли они?                                                                    |    нет   |    частично. доступ ограничен          |
...
| 9   | Разработаны ли процессы и системы Data Governance для управления данными?                                                               |    нет   |     частично         |
...
| 2   | Реализована ли сегментация сети на логические зоны (DMZ, корпоративная сеть, гостевая сеть) для ограничения доступа к критическим ресурсам? |   нет    |     частично, сегментация явно не полная         |
...
| 6   | Установлены ли на серверах и рабочих станциях антивирусные программы и системы EDR?                                                     |   нет    |      частично. нужен аудит        |
